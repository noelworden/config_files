class SchedulePdf < Prawn::Document
  ...
  def find_schedule_count
    # sort occurrences by day
    #

    # { monday: { morning: [], midday: [], evening: [] } }

    schedule = @occurrences.inject(Array.new(7, {})) do |acc, occurrence|
      acc[occurrence.local_start_time.wday][time_of_day(occurrence.local_start_time)] = occurrence
      acc
    end
