...
const getCareerInfo = () => {
  ...
  const createDepartmentOptions = (withParents) => {
    ...
    for (let d in props.departments) {
      if (props.department.hasOwnProperty(d)) {
        let opt = $(generateOption(d))
        let name = opt[0].innerText
        // Dont allow subcategories to be displayed in filtering options
        if (!withParents.includes(name)) {
  ...
  const setUpData = (data) => {
    ...
    data.offices.forEach((l, i) => {
      ...
      l.departments.forEach(d => {
        ...
        // Finding any jobs that are subcategories of a parent industry,
        // then taking the jobs from that subcategory and adding them to the job
        // array of the parent industry
        if (d.parent_id !== null && d.jobs.length > 0) {

          // Pushing subcategory into array to remove from filtering list
          withParents.push(d.name)

          const parentId = _.findKey(l.departments, {'id': d.parent_id})
          // puts the child jobs in the parent jobs
          const combinedJobs = l.departments[parentId].jobs.concat(d.jobs)
          l.departments[parentId].jobs = combinedJobs
          d.jobs = []
        }
