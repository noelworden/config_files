...
module ApplicationHelper
  ...
  def featured_content_path(content, object)
    @featured_class = content[object].class
    if @featured_class == Event
      "http://#{content[object].button_url}"
    elsif @featured_class == Press
      press_path(content[object].slug)
    elsif @featured_class == Resource
      resource_path(content[object].slug)
    else
      case_study_path(content[object].slug)
    end
  end
