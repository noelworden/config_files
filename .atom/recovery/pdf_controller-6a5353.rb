...
class PdfController < ApplicationController
  def week_schedule
    ...
    if is_schedule
      schedule = Schedule.find(params[:schedule]).include(occurrences: [occurrence_program: :studio, program: :category])
      club = Club.find(schedule.club_id)
      filename = "#{club.name}_#{class_type}_classes_week_of_#{date}.pdf"
      # @occurrences = schedule.occurrences.includes(occurrence_program: :studio, program: :category).active
    else
      if params[:schedule_template].present?
        schedule = ScheduleTemplate.find(params[:schedule_template]).includes(occurence_templates: )
        @occurrences = OccurrenceTemplate.by_schedule_template_id(schedule_template.id)
      else
        schedule = ScheduleTemplate.find_by(club_id: params[:club_id], default: true)
        @occurrences = OccurrenceTemplate.by_schedule_template_id(schedule_template.id)
      end
    ...
    date = is_schedule ? Date.commercial(schedule.year, schedule.week).strftime('%Y%m%d') : nil
