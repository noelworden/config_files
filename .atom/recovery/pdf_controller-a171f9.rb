...
class PdfController < ApplicationController
  def week_schedule
    ...
    if is_schedule
      schedule = Schedule.includes(occurrences: [occurrence_program: :studio, program: :category]).find(params[:schedule])
      ...
      schedule = ScheduleTemplate.includes(:occurence_templates).find_by(search_options)
