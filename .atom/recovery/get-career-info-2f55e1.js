...
const getCareerInfo = () => {
  ...
  // ------------------------------------------------
  const createDepartmentOptions = (withParents) => {
    let jobNames = withParents.map(x => x.name)
    // debugger
    ...
    for (let d in props.departments) {
      if ({}.hasOwnProperty.call(props.departments, d)) {
        ...
        let name = opt[0].innerText
        if (_.includes(jobNames, name) === false) {
  ...
  const setUpData = (data) => {
    ...
    data.offices.forEach((l, i) => {
      ...
      l.departments.forEach(d => {
        ...
        if (d.name === 'No Department' || (d.jobs.length === 0 && d.child_ids.length === 0)) return;
        ...
        //Identify an item with a parent_id here, create an array of those items
        //Call that array in createDepartmentOptions to eliminate anything with parent_id
        //Cant eliminate it here bc it needs to work through the rest of this logic to be included in the listings
        ...
        // Finding any jobs that are subcategories of
        if (d.parent_id != null && d.jobs.length > 0) {
          // debugger
          withParents.push(d)
          let childJobs = d.jobs
          let parent_id = _.findKey(l.departments, { 'id': d.parent_id})
          let parentJobs = l.departments[parent_id].jobs
          let combinedJobs = parentJobs.concat(childJobs)
          l.departments[parent_id].jobs = combinedJobs
          d.jobs = []
          // ^ delete entire hash record
          // console.log(_.findKey(l.departments, { 'id': d.parent_id}))
          debugger
        }
    ...
    createDepartmentOptions(withParents);
    // debugger
