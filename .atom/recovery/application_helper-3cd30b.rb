...
module ApplicationHelper
  ...
  def featured_content_path(object)
    record = object.is_a?(Integer)? @featured_content[object] : object

    if record.class == Event
      "http://#{record.button_url}"
    elsif record.class == Press
      press_path(record.slug)
    elsif record.class == Resource
      resource_path(record.slug)
    else
      case_study_path(record.slug)
    end
  end
