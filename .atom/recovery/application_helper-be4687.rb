...
module ApplicationHelper
  ...
  def featured_content_path(object)
    final_object = object.is_a?(Integer)? @featured_content[object] : object

    if final_object.class == Event
      "http://#{final_object.button_url}"
    elsif final_object.class == Press
      press_path(final_object.slug)
    elsif final_object.class == Resource
      resource_path(final_object.slug)
    else
      case_study_path(final_object.slug)
    end
  end
  ...
  def primary_tag(object)
    ...
    final_object.class == CaseStudy ? 'Case Study' : final_object.tag
  end
