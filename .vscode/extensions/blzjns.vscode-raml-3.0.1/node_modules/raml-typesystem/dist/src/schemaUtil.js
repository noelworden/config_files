...
var JSONSchemaObject = (function () {
        ...
        try {
            ...
            if (error.message && error.message.indexOf(illegalRequiredMessageStart) == 0
                ...
                this.acceptErrors(key, [{
                        message: message,
                        params: []
                    }], typesystem_1.messageRegistry.INVALID_JSON_SCHEMA_DETAILS, true, true);
            }
            throw error;
        }
        var missingReferences = validator.getMissingRemoteReferences().filter(function (reference) { return !_.find(alreadyAccepted, function (acceptedReference) { return reference === acceptedReference.reference; }); });
        if (!missingReferences || missingReferences.length === 0) {
            this.acceptErrors(key, validator.getLastErrors(), typesystem_1.messageRegistry.INVALID_JSON_SCHEMA_DETAILS, true, true);
            return;
        }
        var acceptedReferences = [];
        missingReferences.forEach(function (reference) {
    ...
    JSONSchemaObject.prototype.acceptErrors = function (key, errors, regEntry, throwImmediately, isWarning) {
