...
function activate(context) {
    ...
    const disposableCommand = vscode.commands.registerCommand('flexbox.cheatsheet', () => {
        const styleRoot = vscode.Uri.file(path_1.join(context.extensionPath, 'style'));
        const imagesRoot = vscode.Uri.file(path_1.join(context.extensionPath, 'images'));
        const scriptRoot = vscode.Uri.file(path_1.join(context.extensionPath, 'js'));
        ...
        panel.webview.html = webviewContent_1.getWebviewContent(cspSource, scriptPath, stylePath, imagesPath);
    });
...
function buildMarkdownString(context, property) {
    ...
    const filePath = path_1.join(context.extensionPath, 'images', `${property}.svg`);
