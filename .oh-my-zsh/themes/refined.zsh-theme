...
git_dirty() {
    ...
    # Check if it's dirty
    command git diff --quiet --ignore-submodules HEAD &>/dev/null; [ $? -eq 1 ] && echo " %{$fg[white]%}⚡"
}

# Display information about the current repository
#
repo_information() {
    echo "%F{white}${vcs_info_msg_0_%%/.} %F{magenta}$vcs_info_msg_1_`git_dirty` $vcs_info_msg_2_%f"
}
...
precmd() {
    ...
    print -P "\n$(repo_information) %F{blue}$(cmd_exec_time)%f"
